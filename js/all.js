...
(function ajaxGetData() {
  ...
  request.onload = function () {
    ...
    if (request.readyState === 4 && request.status === 200) {
      ...
      for (var i = 0; jsonLen > i; i++) {
        _data.push(json.features[i]);
      }
    }
    var areaData = area.filter(function (element) {
      return element.CityName === '臺北市';
    });
    var pharmacyData = _data.filter(function (element) {
...
function sideBarOpenAndClose() {
  var bar = document.querySelector('.p-sidebar');
...
function clickBar(e) {
  ...
  L.popup().
  setLatLng([lat, lng]).
  setContent("\n\t\t<div class=\"p-card\">\n\t\t\t<div class=\"h-d-flex h-mb-3 h-align-items-center\">\n\t\t\t\t<h2 class=\"h-flex-1\">".concat(


  name, "</h2>\n\t\t\t\t<a class=\"fas fa-location-arrow h-mr-3\" href=\"https://www.google.com.tw/maps/dir//").concat(
  address, "\" target=\"_blank\"></a>\n\t\t\t</div>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

  address, "</span>\n\t\t\t<br>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

  phone, "</span>\n\t\t\t<br>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

  note, "</span>\n\t\t\t<div class=\"h-d-flex h-mt-2\">\n\t\t\t\t<div class=\"p-badges ").concat(

  adultStockNoMore, "\"><span class=\"h5 h-flex-1\">\u6210\u4EBA\u53E3\u7F69</span><span>").concat(adult, "</span></div>\n\t\t\t\t<div class=\"p-badges ").concat(
  childStockNoMore, "\"><span class=\"h5 h-flex-1\">\u5152\u7AE5\u53E3\u7F69</span><span>").concat(child, "</span></div>\n\t\t\t</div>\n\t\t</div>\n\t\t")).
...
"use strict";function addMarker(importData) {
  // marker群組管理
  var markers = new L.MarkerClusterGroup().addTo(_map);
  importData.forEach(function (element) {
    //解構賦值寫法，宣告要從來源變數(element)接收解開的值之變數。
    var geometry = element.geometry,properties = element.properties;
    // 判斷 background-color
    var adultStockNoMore = function () {
    ...
    properties.name, "</h2>\n\t\t    <a class=\"fas fa-location-arrow h-mr-3\" href=\"https://www.google.com.tw/maps/dir//").concat(
    properties.address, "\" target=\"_blank\"></a>\n\t\t  </div>\n\t\t  <span class=\"h4 h-text-dark\">").concat(

    properties.address, "</span>\n\t\t  <br>\n\t\t  <span class=\"h4 h-text-dark\">").concat(

    properties.phone, "</span>\n\t\t  <br>\n\t\t  <span class=\"h4 h-text-dark\">").concat(

    properties.note, "</span>\n\t\t  <div class=\"h-d-flex h-mt-2\">\n\t\t    <div class=\"p-badges ").concat(

    adultStockNoMore, "\"><span class=\"h5 h-flex-1\">\u6210\u4EBA\u53E3\u7F69</span><span>").concat(properties.mask_adult, "</span></div>\n\t\t    <div class=\"p-badges ").concat(
    childStockNoMore, "\"><span class=\"h5 h-flex-1\">\u5152\u7AE5\u53E3\u7F69</span><span>").concat(properties.mask_child, "</span></div>\n\t\t  </div>\n\t\t</div>\n\t\t")));
...
"use strict";function getDate() {
  ...
  var dateStr = "".concat(today.getFullYear(), "-").concat(month, "-").concat(today.getDate());
  console.log(dateStr);
  date.textContent = dateStr;
}
"use strict";function getWeekAndIdCard() {
  // DOM
...
"use strict";function popupOpenOn(importData) {
  var lat = importData[0].geometry.coordinates[1];
  var lng = importData[0].geometry.coordinates[0];
  // 判斷 background-color
  var adultStockNoMore = function () {
  ...
  importData[0].properties.name, "</h2>\n        <a class=\"fas fa-location-arrow h-mr-3\" href=\"https://www.google.com.tw/maps/dir//").concat(
  importData[0].properties.address, "\" target=\"_blank\"></a>\n      </div>\n      <span class=\"h4 h-text-dark\">").concat(

  importData[0].properties.address, "</span>\n      <br>\n      <span class=\"h4 h-text-dark\">").concat(

  importData[0].properties.phone, "</span>\n      <br>\n      <span class=\"h4 h-text-dark\">").concat(

  importData[0].properties.note, "</span>\n      <div class=\"h-d-flex h-mt-2\">\n        <div class=\"p-badges ").concat(

  adultStockNoMore, "\"><span class=\"h4 h-flex-1\">\u6210\u4EBA\u53E3\u7F69</span><span>").concat(importData[0].properties.mask_adult, "</span></div>\n        <div class=\"p-badges ").concat(
  childStockNoMore, "\"><span class=\"h4 h-flex-1\">\u5152\u7AE5\u53E3\u7F69</span><span>").concat(importData[0].properties.mask_child, "</span></div>\n      </div>\n    </div>\n  ")).
...
// }
"use strict";
"use strict";function upDataCounty(areaData) {
  var str = '';
  for (var i = 0; i < areaData.length; i++) {
    str += "\n\t\t<option style=\"border-radius: 20px\" value=\"".concat(
    areaData[i].CityName, "\">").concat(areaData[i].CityName, "</option>\n\t\t");

  }
  county.innerHTML = str;
};
"use strict";function upDataSidebar(importData) {
  var str = '';var _loop = function _loop(
  i) {
    var adultStockNoMore = function () {
    ...
    importData[i].properties.name, "</h2>\n\t\t\t\t<a class=\"h1 h-text-primary fas fa-eye h-mr-3\" id=\"path\" href=\"#\"\n\t\t\t\tdata-lat = \"").concat(

    importData[i].geometry.coordinates[1], "\"\n\t\t\t\tdata-lng = \"").concat(
    importData[i].geometry.coordinates[0], "\"\n\t\t\t\tdata-name = \"").concat(
    importData[i].properties.name, "\"\n\t\t\t\tdata-address = \"").concat(
    importData[i].properties.address, "\"\n\t\t\t\tdata-phone = \"").concat(
    importData[i].properties.phone, "\"\n\t\t\t\tdata-note = \"").concat(
    importData[i].properties.note, "\"\n\t\t\t\tdata-adult = \"").concat(
    importData[i].properties.mask_adult, "\"\n\t\t\t\tdata-child = \"").concat(
    importData[i].properties.mask_child, "\"\n\t\t\t\t></a>\n\t\t\t\t<a class=\"fas fa-location-arrow\" href=\"https://www.google.com.tw/maps/dir//").concat(

    importData[i].properties.address, "\" target=\"_blank\"></a>\n\t\t\t</div>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

    importData[i].properties.address, "</span>\n\t\t\t<br>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

    importData[i].properties.phone, "</span>\n\t\t\t<br>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

    importData[i].properties.note, "</span>\n\t\t\t<br>\n\t\t\t<div class=\"h-d-flex h-mt-2\">\n\t\t\t\t<div class=\"p-badges ").concat(


    adultStockNoMore, "\"><span class=\"h5 h-flex-1\">\u6210\u4EBA\u53E3\u7F69</span><span>").concat(importData[i].properties.mask_adult, "</span></div>\n\t\t\t\t<div class=\"p-badges ").concat(
    childStockNoMore, "\"><span class=\"h5 h-flex-1\">\u5152\u7AE5\u53E3\u7F69</span><span>").concat(importData[i].properties.mask_child, "</span></div>\n\t\t\t</div>\n\t\t</div>\n\t\t");};for (var i = 0; importData.length > i; i++) {_loop(i);
...
"use strict";function upDataTown(areaData) {
  var str = '';
  for (var i = 0; i < areaData[0].AreaList.length; i++) {
    str += "\n\t\t<option style=\"border-radius: 20px\" value=\"".concat(
    areaData[0].AreaList[i].AreaName, "\">\n\t\t").concat(
